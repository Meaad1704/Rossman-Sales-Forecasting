...
def load_xgboost_data():
    # Data for XGBoost prediction and dashboard
    df = pd.read_csv(r'Model Deployment/rossman.csv', parse_dates=['Date'], index_col='Date')
    df = df.sort_index()
...
import os
    ...
    zip_filepath = r'D:\Courses\Data Science - Depi\Final Project\rossmann-store-sales.zip'

    # Folder where the CSV will be extracted
    extract_folder = r'D:\Courses\Data Science - Depi\Final Project\extracted_rossmann'
    os.makedirs(extract_folder, exist_ok=True)

    # Unzip the file (assuming there is only one CSV file named "rossman.csv")
    with zipfile.ZipFile(zip_filepath, 'r') as zip_ref:
        zip_ref.extract('rossman.csv', path=extract_folder)

    # Define the full path to the extracted CSV file
    csv_filepath = os.path.join(extract_folder, 'rossman.csv')

    # Read the CSV and parse the 'Date' column as datetime
    data = pd.read_csv(csv_filepath, parse_dates=['Date'])
